...
const verses = [
  { text: "O Senhor e meu pastor.", reference: "Salmos 23:1" },
  { text: "Posso tudo naquele que me fortalece.", reference: "Filipenses 4:13" },
  { text: "Deus amou o mundo e deu seu Filho.", reference: "Joao 3:16" }
];
...
router.get("/users/all-ids", auth, async (req, res) => {
  try { const r = await pool.query("SELECT id FROM users"); res.json(r.rows.map(x => x.id)); }
