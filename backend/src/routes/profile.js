...
router.patch('/', authenticate, async (req, res) => {
  try {
    ...
    const updated = await db.query('SELECT id, full_name, email, role, avatar_url, cover_url, bio, church_name, location, join_date FROM users WHERE id = $1', [userId]);
...
router.patch('/photo', authenticate, async (req, res) => {
  try {
    ...
    res.json({ success: true, user: { photoURL: updated.rows[0].avatar_url } });
  } catch (err) {
    res.status(500).json({ error: 'Internal server error' });
  }
});
