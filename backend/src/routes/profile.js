...
router.get('/:userId', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id, full_name, email, role, avatar_url, cover_url, bio, church_name, city, country, profession, work, birthdate, marital_status, favorite_verse, testimony, church_denomination, faith_years, ministry FROM users WHERE id = ' + ph + '1', [userId]);
...
router.patch('/photo', authenticate, async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET avatar_url = ' + ph + '1 WHERE id = ' + ph + '2', [photoURL, userId]);
    const updated = await db.query('SELECT avatar_url FROM users WHERE id = ' + ph + '1', [userId]);
...
router.patch('/', authenticate, async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id, full_name, email, role, avatar_url, cover_url, bio, church_name, city, country, profession, marital_status, favorite_verse, testimony, church_denomination, faith_years FROM users WHERE id = ' + ph + '1', [userId]);
