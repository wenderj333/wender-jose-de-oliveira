...
router.get('/:userId', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id, full_name, email, role, avatar_url, cover_url, bio, church_name, city, country, profession, work, birthdate, marital_status, favorite_verse FROM users WHERE id = $1', [userId]);
