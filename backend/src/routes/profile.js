...
router.get('/:userId', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id, full_name, username, email, role, avatar_url, cover_url, bio, church_name, city, state, country, profession, work, birthdate, marital_status, favorite_verse, testimony, church_denomination, faith_years, church_role, ministry FROM users WHERE id = $1', [userId]);
