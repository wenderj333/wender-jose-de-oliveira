...
router.patch('/', authenticate, async (req, res) => {
  try {
    ...
    await db.query('UPDATE users SET full_name=COALESCE(NULLIF(\,\'\'), full_name), bio=COALESCE(NULLIF(\,\'\'), bio), city=COALESCE(NULLIF(\,\'\'), city), country=COALESCE(NULLIF(\,\'\'), country), profession=COALESCE(NULLIF(\,\'\'), profession), marital_status=COALESCE(NULLIF(\,\'\'), marital_status), church_name=COALESCE(NULLIF(\,\'\'), church_name), church_denomination=COALESCE(NULLIF(\,\'\'), church_denomination), faith_years=COALESCE(NULLIF(\,\'\'), faith_years), favorite_verse=COALESCE(NULLIF(\,\'\'), favorite_verse), testimony=COALESCE(NULLIF(\,\'\'), testimony), avatar_url=COALESCE(NULLIF(\,\'\'), avatar_url), cover_url=COALESCE(NULLIF(\,\'\'), cover_url), updated_at=NOW() WHERE id=', [full_name, bio, city, country, profession, marital_status, church_name, denomination, christian_years, favorite_verse, testimony, avatar_url, cover_url, userId]);
