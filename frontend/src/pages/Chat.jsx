...
export default function Chat() {
  ...
  const resetCallMedia = () => { pendingCandidatesRef.current = []; peerRef.current?.close(); peerRef.current = null; localStreamRef.current?.getTracks().forEach(track => track.stop()); localStreamRef.current = null; };
  const closeCall = (notify = true) => { const active = callRef.current; if (notify && active?.id) sendSocket?.({ type: 'call_end', callId: active.id }); if (callTimeoutRef.current) clearTimeout(callTimeoutRef.current); callTimeoutRef.current = null; resetCallMedia(); setCurrentCall(null); };
  const mediaErrorMessage = (error, mode = 'audio') => {
    if (!navigator.mediaDevices?.getUserMedia) return 'Este navegador não permite chamadas de voz ou vídeo.';
    if (error?.name === 'NotAllowedError' || error?.name === 'SecurityError') return mode === 'video' ? 'Permita o microfone e a câmara para a videochamada e tente novamente.' : 'Permita o microfone para a chamada de voz e tente novamente.';
  ...
  const acceptCall = async () => { const active = callRef.current; if (!active) return; try { await setupCall(active, false); sendSocket?.({ type: 'call_response', callId: active.id, accepted: true }); } catch (error) { resetCallMedia(); setCurrentCall({ ...active, status: 'incoming', error: mediaErrorMessage(error, active.mode) }); } };
    ...
    <div style={{
      ...
      {call && <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(15,23,42,.78)', display: 'grid', placeItems: 'center', padding: 16 }}>
        <div style={{ width: 'min(680px,100%)', background: '#101b33', color: '#fff', borderRadius: 20, padding: 20, textAlign: 'center', boxShadow: '0 22px 70px rgba(0,0,0,.35)' }}>
          ...
          {call.error && <p style={{ margin:'-6px auto 16px', maxWidth:440, padding:'10px 12px', borderRadius:10, background:'rgba(192,57,43,.18)', color:'#fff', fontWeight:650, lineHeight:1.4 }}>{call.error}</p>}
