...
export default function Landing() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#ffffff',fontFamily:"'Inter',system-ui,sans-serif",overflowX:'hidden'}}>
      ...
      {/* HERO */}
      <section style={{paddingTop:100,paddingBottom:72,background:'linear-gradient(180deg,#f0f5ff 0%,#ffffff 100%)',textAlign:'center',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{transform:`translateY(${animY}px)`,transition:'transform 0.1s',display:'inline-block',marginBottom:24}}>
          <div style={{width:88,height:88,borderRadius:'50%',background:'linear-gradient(135deg,#3568b8 0%,#4a80d4 55%,#6a9ade 100%)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'2.8rem',boxShadow:'0 12px 36px rgba(74,128,212,0.28)',margin:'0 auto'}}>📖</div>
        </div>

        <div style={{width:56,height:3,background:'linear-gradient(90deg,transparent,#a07820,transparent)',margin:'0 auto 18px',borderRadius:2}}/>

        <h1 style={{fontFamily:"'Cormorant Garamond',Georgia,serif",fontSize:'clamp(2.2rem,6vw,3.8rem)',fontWeight:700,color:'#1e2240',marginBottom:14,lineHeight:1.15,padding:'0 20px'}}>
          Sigo com Fé<br/>
          <span style={{background:'linear-gradient(135deg,#3568b8,#4a80d4)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',fontSize:'clamp(1.3rem,3.5vw,2rem)',fontWeight:600}}>
            {t('landing.tagline')}
          </span>
        </h1>

        <p style={{fontSize:'clamp(0.95rem,2.2vw,1.15rem)',color:'#4a5568',maxWidth:520,margin:'0 auto 12px',lineHeight:1.75,padding:'0 24px'}}>
          {t('landing.subtitle')}
        </p>

        <p style={{fontSize:'0.88rem',color:'#a07820',fontStyle:'italic',marginBottom:34,fontWeight:500,padding:'0 24px'}}>
          {t('landing.verse')}
        </p>

        <div style={{display:'flex',gap:14,justifyContent:'center',flexWrap:'wrap',padding:'0 20px',marginBottom:48}}>
          <Link to="/register" style={{padding:'13px 34px',borderRadius:12,background:'linear-gradient(135deg,#3568b8,#4a80d4)',color:'white',textDecoration:'none',fontWeight:700,fontSize:'1rem',boxShadow:'0 6px 22px rgba(74,128,212,0.35)',letterSpacing:'0.02em'}}>
            {t('landing.createFree')}
          </Link>
          <Link to="/login" style={{padding:'13px 34px',borderRadius:12,background:'white',border:'2px solid #4a80d4',color:'#4a80d4',textDecoration:'none',fontWeight:700,fontSize:'1rem',boxShadow:'0 4px 12px rgba(74,128,212,0.1)'}}>
      ...
      <section style={{padding:'72px 24px',background:'#ffffff'}}>
        <div style={{maxWidth:980,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(270px,1fr))',gap:18}}>
            {features.map(f => (
              <div key={f.title} style={{background:'#f8faff',borderRadius:16,padding:26,border:'1px solid #e0e6f5',cursor:'default',transition:'transform 0.2s,box-shadow 0.2s'}}
                ...
                <div style={{width:50,height:50,borderRadius:13,background:`${f.color}15`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.6rem',marginBottom:14,border:`1px solid ${f.color}20`}}>
                  {f.emoji}
                </div>
                <h3 style={{fontSize:'1rem',fontWeight:700,color:'#1e2240',marginBottom:7}}>{f.title}</h3>
      ...
      {/* TESTIMONIALS */}
      <section style={{padding:'72px 24px',background:'linear-gradient(135deg,#f0f5ff 0%,#e8eeff 100%)'}}>
        <div style={{maxWidth:880,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:18}}>
            {testimonials.map(t2 => (
              <div key={t2.name} style={{background:'white',borderRadius:16,padding:26,border:'1px solid #dde3f0',boxShadow:'0 4px 14px rgba(74,128,212,0.06)'}}>
                <div style={{fontSize:'1.7rem',marginBottom:10}}>{t2.avatar}</div>
      ...
      <section style={{padding:'72px 24px',background:'linear-gradient(135deg,#3568b8 0%,#4a80d4 55%,#6a9ade 100%)',textAlign:'center',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1}}>
          ...
          <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(1.8rem,4.5vw,2.8rem)',fontWeight:700,color:'white',marginBottom:14,lineHeight:1.25,padding:'0 20px'}}>
            {t('landing.ctaTitle')}
          </h2>
          <p style={{fontSize:'1rem',color:'rgba(255,255,255,0.85)',marginBottom:32,maxWidth:460,margin:'0 auto 32px',padding:'0 20px'}}>
            {t('landing.ctaSubtitle')}
          </p>
          <Link to="/register" style={{display:'inline-block',padding:'15px 46px',borderRadius:14,background:'white',color:'#3568b8',textDecoration:'none',fontWeight:700,fontSize:'1.05rem',boxShadow:'0 8px 24px rgba(0,0,0,0.18)',letterSpacing:'0.02em'}}>
            {t('landing.ctaButton')}
          </Link>
          <p style={{color:'rgba(255,255,255,0.55)',fontSize:'0.8rem',marginTop:14}}>
            {t('landing.ctaFree')}
          </p>
        </div>
      </section>
      ...
      {/* FOOTER */}
      <footer style={{background:'#1e2240',padding:'26px 24px',textAlign:'center'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:8}}>
          <span style={{fontSize:'1.2rem'}}>📖</span>
          <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.05rem',fontWeight:700,color:'white'}}>Sigo com Fé</span>
        </div>
