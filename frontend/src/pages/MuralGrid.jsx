...
function DailySurpriseBoxes() {
  ...
  const words = [
    { ref: 'Isaías 41:10', pt: 'Não temas, porque eu sou contigo.', es: 'No temas, porque yo estoy contigo.', en: 'Do not fear, for I am with you.' },
    { ref: 'Salmos 46:1', pt: 'Deus é o nosso refúgio e fortaleza.', es: 'Dios es nuestro refugio y nuestra fuerza.', en: 'God is our refuge and strength.' },
    { ref: 'Provérbios 3:5-6', pt: 'Confia no Senhor de todo o teu coração.', es: 'Confía en el Señor de todo tu corazón.', en: 'Trust in the Lord with all your heart.' },
    { ref: 'Filipenses 4:13', pt: 'Tudo posso naquele que me fortalece.', es: 'Todo lo puedo en aquel que me fortalece.', en: 'I can do all things through Christ who strengthens me.' },
  ];
  const copy = { choose: { pt: 'Escolha uma surpresa', es: 'Elige una sorpresa', en: 'Choose a surprise' }, done: { pt: 'Você já recebeu sua palavra hoje', es: 'Ya recibiste tu palabra de hoy', en: "You already received today's word" }, share: { pt: 'Compartilhar esta palavra', es: 'Compartir esta palabra', en: 'Share this word' } };
  useEffect(() => { try { const saved = JSON.parse(localStorage.getItem(storageKey)); if (saved) setMessage(saved); } catch (_) {} }, [storageKey]);
  const choose = box => { if (message) return; const word = words[Math.floor(Math.random() * words.length)]; const value = { ...word, box: box.title.pt }; setMessage(value); localStorage.setItem(storageKey, JSON.stringify(value)); };
  const share = async () => { if (!message) return; const body = `${message[lang] || message.pt}\n— ${message.ref}\nSigo com Fé`; try { if (navigator.share) await navigator.share({ title: 'Palavra do dia', text: body }); else await navigator.clipboard.writeText(body); } catch (_) {} };
  return <section aria-label={copy.choose[lang] || copy.choose.pt} style={{ marginBottom: 20, padding: '18px 16px', borderRadius: 18, background: 'linear-gradient(135deg,#fffaf1,#f6f3ff)', border: '1px solid #eadff3', boxShadow: '0 8px 24px rgba(70,45,100,.08)' }}><div style={{ textAlign: 'center', marginBottom: 14 }}><div style={{ fontSize: 22 }}>✨</div><h2 style={{ margin: '2px 0 3px', color: '#30204f', fontSize: 19 }}>{message ? (copy.done[lang] || copy.done.pt) : (copy.choose[lang] || copy.choose.pt)}</h2><p style={{ margin: 0, color: '#756b80', fontSize: 12 }}>{message ? (message[lang] || message.pt) : 'Uma palavra especial para a sua caminhada'}</p></div>{!message ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 10 }}>{boxes.map(box => <button key={box.title.pt} type="button" onClick={() => choose(box)} style={{ border: 'none', borderRadius: 14, padding: '15px 8px', color: 'white', background: `linear-gradient(145deg,${box.color},${box.color}cc)`, cursor: 'pointer', minHeight: 105, boxShadow: '0 6px 14px rgba(56,35,80,.16)', transition: 'transform .2s' }} onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; }} onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; }}><span style={{ display: 'block', fontSize: 28, marginBottom: 6 }}>{box.icon}</span><strong style={{ display: 'block', fontSize: 14 }}>{box.title[lang] || box.title.pt}</strong><small style={{ display: 'block', marginTop: 5, opacity: .9, lineHeight: 1.2 }}>{box.hint[lang] || box.hint.pt}</small></button>)}</div> : <div style={{ textAlign: 'center' }}><div style={{ color: '#7c6d90', fontSize: 12, marginBottom: 12 }}>{message.ref}</div><button type="button" onClick={share} style={{ border: 'none', borderRadius: 20, padding: '9px 17px', color: 'white', background: '#6d47a8', cursor: 'pointer', fontWeight: 700 }}>↗ {copy.share[lang] || copy.share.pt}</button></div>}</section>;
}
