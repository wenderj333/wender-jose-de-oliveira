      ...
      <div style={{ padding: '0 24px', marginTop: -50, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {isOwn ? (
                ...
                <div style={{ position: 'relative' }}>
                  <button onClick={() => setShowMenu(!showMenu)} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #dbdbdb', background: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 18 }}>Ôï«</button>
                  {showMenu && (
                    <div style={{ position: 'absolute', top: 40, right: 0, background: '#fff', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', zIndex: 100, minWidth: 200, overflow: 'hidden' }}>
                      <button onClick={() => { navigator.clipboard.writeText(window.location.href); setShowMenu(false); }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>­ƒöù {t('profile.copyLink')}</button>
                      <button onClick={() => { coverRef.current?.click(); setShowMenu(false); }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>­ƒôÀ {t('profile.changeCover')}</button>
                      <button onClick={() => { avatarRef.current?.click(); setShowMenu(false); }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>­ƒû╝´©Å {t('profile.changeAvatar')}</button>
                      <button onClick={() => { setProfile(prev => ({...prev, cover_url: null})); setShowMenu(false); }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 14, color: '#e11d48', display: 'flex', alignItems: 'center', gap: 8 }}>­ƒùæ´©Å {t('profile.deleteCover')}</button>
                        <div style={{height:1,background:'#f0f0f0',margin:'4px 0'}}/>
                        <button onClick={() => { setShowMenu(false); if(window.confirm('Tem certeza? Esta acao nao pode ser desfeita.')) { fetch(API_BASE + '/api/auth/delete-account', { method: 'DELETE', headers: { Authorization: 'Bearer ' + token } }).then(() => { logout(); }); } }} style={{ width: '100%', padding: '12px 16px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 14, color: '#e11d48', display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700 }}>ÔÜá´©Å Apagar Conta</button>
                    </div>
                  )}
                </div>
                </>
        ...
        {activeTab === 'prayers' && (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#888' }}>
            ...
            <p>Pedidos de ora├º├úo em breve</p>
          </div>
        )}
      ...
      {editMode && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 480, padding: 24 }}>
            ...
            {[['Nome completo', 'full_name'], ['Bio', 'bio'], ['Localiza├º├úo', 'location'], ['Igreja', 'church_name']].map(([label, key]) => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setSelectedPost(null)}>
          <div style={{ background: '#fff', borderRadius: 16, maxWidth: 800, width: '100%', maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 16, color: '#888', fontSize: 13 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Heart size={16} /> {selectedPost.like_count || 0} Am├®n</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><MessageCircle size={16} /> {selectedPost.comment_count || 0} Coment├írios</span>
              </div>
            </div>
          </div>
        </div>
