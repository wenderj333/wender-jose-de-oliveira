...
export default function Profile() {
  ...
  React.useEffect(() => {
    if (profile) setSpiritualData({
      ...
      christian_values: profile.christian_values || '',
  ...
  async function saveSpiritualProfile() {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setProfile(prev => ({ ...prev, ...spiritualData }));
        setEditSpiritual(false);
      }
      ...
      {profile && (
        <div style={{ margin: '16px 0', background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: '#1a1a2e', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            ✝️ Perfil Espiritual
            {isOwner && !editSpiritual && <button onClick={() => setEditSpiritual(true)} style={{ marginLeft: 'auto', fontSize: 11, padding: '4px 12px', borderRadius: 20, border: '1px solid #d0dbff', background: '#fff', cursor: 'pointer', color: '#3a6ad4' }}>Editar</button>}
          </div>
          {editSpiritual ? (
            <div>
              {[
                ['favorite_verse', '📖 Versículo favorito'],
                ['testimony', '🙏 Testemunho pessoal'],
                ['life_motto', '💬 Frase ou lema de vida'],
                ['church_denomination', '⛪ Igreja / Denominação'],
                ['faith_years', '📅 Tempo na fé'],
                ['platform_purpose', '🎯 Propósito na plataforma'],
                ['spiritual_gifts', '🎁 Dons espirituais'],
                ['interest_areas', '❤️ Áreas de interesse'],
                ['christian_values', '✨ Valores cristãos'],
              ].map(([field, label]) => (
                <div key={field} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 11, color: '#888', marginBottom: 4, fontWeight: 600 }}>{label}</div>
                  <textarea value={spiritualData[field] || ''} onChange={e => setSpiritualData(prev => ({ ...prev, [field]: e.target.value }))}
                    style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid #d0dbff', fontSize: 12, resize: 'vertical', minHeight: 48, fontFamily: 'inherit', outline: 'none' }} />
                </div>
              ))}
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 11, color: '#888', marginBottom: 4, fontWeight: 600 }}>🌱 Estado espiritual</div>
                <select value={spiritualData.spiritual_state || 'em crescimento'} onChange={e => setSpiritualData(prev => ({ ...prev, spiritual_state: e.target.value }))}
                  style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid #d0dbff', fontSize: 12 }}>
            ...
            <div>
              {profile.favorite_verse && <div style={{ marginBottom: 10 }}><div style={{ fontSize: 10, color: '#888', fontWeight: 600, marginBottom: 2 }}>📖 VERSÍCULO FAVORITO</div><div style={{ fontSize: 12, color: '#333', fontStyle: 'italic', lineHeight: 1.5 }}>{profile.favorite_verse}</div></div>}
              ...
              {profile.life_motto && <div style={{ marginBottom: 10 }}><div style={{ fontSize: 10, color: '#888', fontWeight: 600, marginBottom: 2 }}>💬 LEMA DE VIDA</div><div style={{ fontSize: 12, color: '#333', fontStyle: 'italic' }}>{profile.life_motto}</div></div>}
              ...
              {profile.spiritual_state && <div style={{ marginBottom: 10 }}><div style={{ fontSize: 10, color: '#888', fontWeight: 600, marginBottom: 2 }}>🌱 ESTADO ESPIRITUAL</div><span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, background: '#ede9fe', color: '#7c3aed', fontWeight: 600 }}>{profile.spiritual_state}</span></div>}
              ...
              {!profile.favorite_verse && !profile.testimony && isOwner && <div style={{ fontSize: 12, color: '#aaa', textAlign: 'center', padding: 10 }}>Clica em Editar para preencher o teu perfil espiritual!</div>}
            </div>
          )}
        </div>
      )}
