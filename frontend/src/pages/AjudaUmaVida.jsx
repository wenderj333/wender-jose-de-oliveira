        ...
        <div style={{background:"linear-gradient(135deg,#7a9e7e,#8fac7a)",borderRadius:16,padding:20,margin:"0 16px 20px",color:"white"}}>
          ...
          {(postType==="request" || postType==="offer") && (
            <div style={{marginTop:8,marginBottom:4}}>
              <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',background:'rgba(255,255,255,0.1)',border:'1px solid rgba(255,255,255,0.2)',borderRadius:12,padding:10}}>
                <span style={{fontSize:13,color:'white'}}>📸 {mediaFile ? mediaFile.name : 'Adicionar foto/vídeo (opcional)'}</span>
                <input type='file' accept='image/*,video/*' style={{display:'none'}} onChange={e=>setMediaFile(e.target.files[0])}/>
              </label>
              {mediaFile && <button onClick={()=>setMediaFile(null)} style={{marginTop:4,fontSize:11,color:'rgba(255,255,255,0.6)',background:'none',border:'none',cursor:'pointer'}}>✕ Remover</button>}
            </div>
          ...
          return (
            <div key={post.id} style={{background:"white",borderRadius:16,padding:16,marginBottom:14,boxShadow:"0 2px 12px rgba(0,0,0,0.06)",border:post.is_urgent?"2px solid #e74c3c":"1px solid #f0eaff",position:"relative"}}>
              ...
              {post.media_url && (post.media_url.includes("video") ? (
                <video src={post.media_url} controls style={{width:"100%",borderRadius:12,marginBottom:12,maxHeight:300}} />
              ) : (
                <img src={post.media_url} style={{width:"100%",borderRadius:12,marginBottom:12,maxHeight:300,objectFit:"cover"}} />
              ))}
