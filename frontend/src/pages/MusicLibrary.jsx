...
import {
  ...
  MessageCircle, TrendingUp, Clock, ThumbsUp,
} from 'lucide-react';
...
// ─── Comments Modal ──────────────────────────────────────────────────────────
function CommentsModal({ song, onClose, token, user }) {
  ...
  const fetchComments = async () => {
    try {
      const res = await fetch(`${API}/music/${song.id}/comments`);
  ...
  return (
    <div style={{
      ...
      <div style={{ background: 'var(--card, #fff)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 500, maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>💬 Comentários</h3>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
            <X size={20} color="#888" />
          </button>
        </div>

        <div style={{ marginBottom: 12, padding: 12, background: 'var(--bg, #f8f9fa)', borderRadius: 12 }}>
          <p style={{ margin: 0, fontWeight: 600, fontSize: 14, color: 'var(--text)' }}>{song.title}</p>
          <p style={{ margin: '2px 0 0', fontSize: 12, color: '#888' }}>{song.artist || song.user_name}</p>
        </div>
        ...
        {user && (
          <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <input
              ...
              style={{
                flex: 1, padding: '10px 12px', borderRadius: 10, border: '1px solid var(--border, #e2e8f0)',
                outline: 'none', fontSize: 14, background: 'var(--bg, #fff)', color: 'var(--text)',
              }}
            />
            <button type="submit" disabled={!newComment.trim()} style={{
              padding: '10px 16px', borderRadius: 10, border: 'none',
              background: newComment.trim() ? 'var(--fb, #4a80d4)' : '#ccc',
              color: '#fff', fontWeight: 600, cursor: newComment.trim() ? 'pointer' : 'not-allowed',
            }}>
              Enviar
            </button>
          </form>
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem', color: '#888' }}>
              ...
              <p>Nenhum comentário ainda.<br />Sê o primeiro! 💬</p>
            </div>
          ) : (
            comments.map(c => (
              <div key={c.id} style={{ display: 'flex', gap: 10, padding: 12, background: 'var(--bg, #f8f9fa)', borderRadius: 12 }}>
                <div style={{
                  width: 36, height: 36, borderRadius: '50%', flexShrink: 0,
                  background: c.user_avatar ? undefined : 'linear-gradient(135deg, #4a80d4, #764ba2)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 16, color: '#fff', overflow: 'hidden',
                }}>
                  {c.user_avatar ? <img src={c.user_avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : c.user_name?.[0]?.toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontWeight: 600, fontSize: 13, color: 'var(--text)' }}>{c.user_name}</p>
                  <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--text, #333)' }}>{c.comment}</p>
    ...
    <div ref={ref} style={{ position: 'relative', flex: 1 }}>
      ...
      {open && (
        <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', bottom: '110%', left: 0, right: 0, background: 'var(--card,#fff)', border: '1px solid var(--border,#e2e8f0)', borderRadius: 10, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', zIndex: 100, minWidth: 140 }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); const txt = encodeURIComponent('🎵 ' + song.title + (song.artist ? ' - ' + song.artist : '') + '\n🙏 Partilhado via Sigo com Fe - Rede Social Crista\n🔗 https://sigo-com-fe-api.onrender.com/api/music/share/' + song.id); window.open('https://wa.me/?text=' + txt, '_blank'); setOpen(false); }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#25D166' }}>
            📱 WhatsApp
          </button>
          <button onClick={(e) => { e.stopPropagation(); window.open('https://t.me/share/url?url=' + encodeURIComponent('https://sigo-com-fe-api.onrender.com/api/music/share/' + song.id) + '&text=' + encodeURIComponent('🎵 ' + song.title + (song.artist ? ' - ' + song.artist : '') + '\n🙏 Sigo com Fe - Rede Social Crista'), '_blank'); setOpen(false); }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#0088cc' }}>
            ✈️ Telegram
          </button>
          <button onClick={(e) => { e.stopPropagation(); window.open('https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent('https://sigo-com-fe-api.onrender.com/api/music/share/' + song.id), '_blank'); setOpen(false); }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#1877F2' }}>
            🇫🇧 Facebook
          </button>
          <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(song.url); alert('📸 Link copiado! Cola no Instagram Stories ou Bio.'); setOpen(false); }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#E1306C' }}>
            📸 Instagram
          </button>
          <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText('https://sigo-com-fe-api.onrender.com/api/music/share/' + song.id); alert('✅ Link copiado!'); setOpen(false); }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#555' }}>
            🔗 Copiar Link
          </button>
        </div>
      )}
...
// ─── Song Card ────────────────────────────────────────────────────────────────
function SongCard({ song, isPlaying, onPlay, onDelete, onPublish, onComment, token, user, t }) {
    ...
    <div style={{
      ...
      {isPlaying && !isVideo && (
        <div style={{
          ...
          background: 'var(--fb, #4a80d4)', borderRadius: 8, padding: '4px 8px',
          display: 'flex', alignItems: 'center', gap: 6,
        }}>
          <Equalizer />
          <span style={{ fontSize: 10, color: '#fff', fontWeight: 600 }}>TOCANDO</span>
        </div>
      )}
      ...
      <div style={{ fontSize: 11, color: '#888', marginBottom: 4 }}>{song.artist || song.user_name}</div>

      {/* Stats row */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 8, fontSize: 11, color: '#888' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
          <Play size={11} /> {song.play_count || 0}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); onComment(song); }} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: '1px solid var(--border)', borderRadius: 8, padding: '5px 8px', cursor: 'pointer', color: '#888', fontSize: 11, fontWeight: 600 }}>
        ...
        {user && (
          <div style={{ display: 'flex', gap: 6 }}>
            <ShareMenu song={song} onPublish={handlePublishClick} />
            {canDelete && (
              <button onClick={handleDeleteClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, background: 'rgba(225,29,72,0.08)', border: '1px solid rgba(225,29,72,0.2)', borderRadius: 8, padding: '5px 8px', cursor: 'pointer', color: '#e11d48', fontSize: 11, fontWeight: 600 }}>
                <Trash2 size={12} />
              </button>
            )}
          </div>
...
export default function MusicLibrary() {
  ...
  const fetchSongs = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/music?sort=${sortBy}`, { headers });
      const data = await res.json();
  ...
  return (
    <>
        ...
        <div style={{ padding: '0 1rem', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{
              ...
              fontSize: 13, fontWeight: 600, background: 'var(--card, #fff)', color: 'var(--text)',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {filteredSongs.map((song, idx) => (
                <SongCard 
                  key={song.id} 
                  song={song} 
                  isPlaying={currentIdx === idx && playing && !isVideoUrl(song.url)} 
                  onPlay={handlePlay} 
                  onDelete={handleDelete} 
                  onPublish={handlePublish} 
                  onComment={(s) => setShowComments(s)}
                  token={token} 
                  user={user} 
                  t={t} 
                />
                    ...
                    {isVid ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <div onClick={() => handlePlay(song)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', cursor: 'pointer' }}>
                          <div style={{ width: 48, height: 48, borderRadius: 10, flexShrink: 0, overflow: 'hidden', background: 'linear-gradient(135deg,#4a80d4,#764ba2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, position: 'relative' }}>
                            {song.cover_url ? <img src={song.cover_url} alt={song.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : '🎵'}
                            {isActive && (
                              <div style={{ position: 'absolute', inset: 0, background: 'rgba(74,128,212,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <Equalizer />
                              </div>
                            )}
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p style={{ margin: 0, fontWeight: 600, fontSize: '0.88rem', color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{song.title}</p>
                            <p style={{ margin: '2px 0 0', fontSize: '0.75rem', color: '#888' }}>{song.artist || song.user_name}</p>
                            <div style={{ display: 'flex', gap: 8, marginTop: 2, fontSize: '0.7rem', color: '#999' }}>
                              <span>▶️ {song.play_count || 0}</span>
                        ...
                        {user && (
                          <div style={{ display: 'flex', gap: 6, padding: '0 14px 10px' }}>
                            <button onClick={() => setShowComments(song)} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(74,128,212,0.1)', border: '1px solid rgba(74,128,212,0.3)', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', color: 'var(--fb)', fontSize: 11, fontWeight: 600 }}>
                              <MessageCircle size={12} /> Comentar
                            </button>
                            ...
                            <button onClick={() => { const txt = encodeURIComponent('🎵 ' + song.title + (song.artist ? ' - ' + song.artist : '') + '\n🙏 Partilhado via Sigo com Fe - Rede Social Crista\n🔗 https://sigo-com-fe-api.onrender.com/api/music/share/' + song.id); window.open('https://wa.me/?text=' + txt, '_blank'); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.3)', borderRadius: 8, padding: '5px 8px', cursor: 'pointer', color: '#25D166', fontSize: 11, fontWeight: 600 }}>
                              <Share2 size={12} /> WA
                            </button>
      ...
      {showComments && (
        <CommentsModal song={showComments} onClose={() => setShowComments(null)} token={token} user={user} />
      )}
