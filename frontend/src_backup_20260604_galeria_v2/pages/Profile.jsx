    ...
    <div style={{ maxWidth: "935px", margin: "0 auto", padding: "0" }}>
      ...
      {activeTab !== "galeria" && (
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:3,marginTop:3}}>
          {[...userPosts.filter(p=>activeTab==="all"?true:activeTab==="foto"?(p.media_url&&!p.media_url.includes("video")):(p.media_url&&p.media_url.includes("video"))),
            ...
            <div style={{marginBottom:12}}>
              ...
              {showUpload && (
                <div style={{background:"#f9f9fe",borderRadius:12,padding:16,marginTop:10,border:"1px solid #eee"}}>
                  ...
                  <button onClick={()=>photoInputRef.current?.click()} disabled={uploading} style={{background:"#6C3FA0",color:"white",border:"none",borderRadius:8,padding:"10px 20px",cursor:"pointer",fontSize:14,marginTop:8}}>{uploading?t("profile.uploading","A fazer upload..."):"📤 "+t("profile.publishPhoto","Publicar Foto")}</button>
                </div>
              )}
            </div>
          )}
