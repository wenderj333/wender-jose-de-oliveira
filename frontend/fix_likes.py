with open(r'C:\Users\wender\Desktop\SIGO COM FE\SIGO COM FE LOCAL\sigo-com-fe\frontend\src\pages\Profile.jsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Melhorar handlePostAmen para guardar e mostrar contagem
old_amen = """  const [postAmens, setPostAmens] = useState({});"""
new_amen = """  const [postAmens, setPostAmens] = useState({});
  const [postLikeCounts, setPostLikeCounts] = useState({});"""
content = content.replace(old_amen, new_amen)

# Melhorar a funcao de like
old_func = """  const handlePostAmen = async (postId) => {
    if (postAmens[postId]) return;
    setPostAmens(prev => ({ ...prev, [postId]: true }));
    try {
      await fetch(API + "/feed/" + postId + "/like", { method: "POST", headers: { Authorization: "Bearer " + token } });
    } catch(e) {}
  };"""

new_func = """  const handlePostAmen = async (postId, currentCount) => {
    const alreadyLiked = postAmens[postId];
    setPostAmens(prev => ({ ...prev, [postId]: !alreadyLiked }));
    setPostLikeCounts(prev => ({ ...prev, [postId]: (prev[postId] ?? currentCount ?? 0) + (alreadyLiked ? -1 : 1) }));
    try {
      await fetch(API + "/feed/" + postId + "/like", { method: "POST", headers: { Authorization: "Bearer " + token } });
    } catch(e) {
      setPostAmens(prev => ({ ...prev, [postId]: alreadyLiked }));
      setPostLikeCounts(prev => ({ ...prev, [postId]: (prev[postId] ?? currentCount ?? 0) + (alreadyLiked ? 1 : -1) }));
    }
  };"""
content = content.replace(old_func, new_func)

# Adicionar contagem no botao Amen
old_btn = """              <button onClick={(e) => { e.stopPropagation(); handlePostAmen(p.id); }} style={{ position: "absolute", bottom: 6, left: 6, background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 3, color: "white", textShadow: "0 1px 3px rgba(0,0,0,0.8)", fontSize: 12, fontWeight: 700 }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill={postAmens[p.id] ? "#e11d48" : "none"} stroke="white" strokeWidth="2"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
              </button>"""

new_btn = """              <button onClick={(e) => { e.stopPropagation(); handlePostAmen(p.id, p.like_count); }} style={{ position: "absolute", bottom: 6, left: 6, background: "rgba(0,0,0,0.3)", border: "none", borderRadius: 20, padding: "3px 8px", cursor: "pointer", display: "flex", alignItems: "center", gap: 3, color: "white", fontSize: 11, fontWeight: 700 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill={postAmens[p.id] ? "#e11d48" : "none"} stroke="white" strokeWidth="2"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                {postLikeCounts[p.id] ?? p.like_count ?? 0}
              </button>"""

content = content.replace(old_btn, new_btn)

with open(r'C:\Users\wender\Desktop\SIGO COM FE\SIGO COM FE LOCAL\sigo-com-fe\frontend\src\pages\Profile.jsx', 'w', encoding='utf-8') as f:
    f.write(content)

print('Like count: ' + ('OK' if 'postLikeCounts' in content else 'FALHOU'))
